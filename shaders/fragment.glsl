...
void main() {
	...
	float step1 =  ceil(newUV.y * 5.) / 5.; 
	float scroll = (time * 0.001) * (.5 + hash1(step1));
	...
	newUV = (newUV - vec2(0.5, step1 - 0.1)) * (vec2(1. + 0.1 * shade, 1. + 0.7 * shade) - 0.6 * (1. - progress)) + vec2(0.5, step1 - 0.1);

	float direction = (mod(ceil(newUV.y * 5.), 2.) == 0.) ?  -1. : 1.;

	newUV.x = mod(newUV.x - scroll + 0.3 * progress * direction + hash1(step1), 1.);
